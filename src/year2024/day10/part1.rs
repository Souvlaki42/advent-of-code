...
fn find_trailhead_score(grid: &[Vec<char>], x: usize, y: usize) -> usize {
    let mut trails: Vec<(usize, usize)> = Vec::new();
                        ...
                        for six in found_six.iter() {
                            ...
                            for seven in found_seven.iter() {
                                ...
                                for eight in found_eight.iter() {
                                    ...
                                    for nine in found_nine.iter() {
                                        if !trails.contains(&(nine.0, nine.1)) {
                                            trails.push((nine.0, nine.1));
                                        }
                                    }
                                }
                            }
...
inventory::submit! {
    crate::Solution { year: 2024, day: 10, part: 1, run: run }
}
...
fn run() {
    ...
    for x in 0..grid.len() {
        for y in 0..grid[0].len() {
            if grid[x][y] == '0' {
                sum += find_trailhead_score(&grid, x, y);
            }
        }
    }
